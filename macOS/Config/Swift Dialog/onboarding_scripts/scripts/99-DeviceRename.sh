...
if [ "$?" = "0" ]; then
  echo " $(date) | Serial detected as $SerialNum"
   ...
   updateSplashScreen fail "Unable to determine serial number"
   exit 1
fi
...
if [ "$?" = "0" ]; then
  echo " $(date) | Current computername detected as $CurrentNameCheck"
   ...
   updateSplashScreen fail "Unable to determine current name"
   exit 1
fi
...
echo " $(date) | Company variable set to $CompanyCode"
...
if [[ "$CurrentNameCheck" == "$NewName" ]]
  ...
  echo " $(date) | Rename not required already set to [$CurrentNameCheck]"
   updateSplashScreen success "Already set to $CurrentNameCheck"
  exit 0
fi
...
updateSplashScreen success "Renamed to $NewName"